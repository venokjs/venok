import type { MessageEvent } from "~/interfaces/index.js";

import { IncomingMessage, OutgoingHttpHeaders } from "http";
import { Transform } from "node:stream";
import { isObject } from "@venok/core";
...
export class SseStream extends Transform {
  ...
  pipe<T extends WritableHeaderStream>(
    ...
    if (destination.writeHead) {
      ...
      if (destination.flushHeaders) destination.flushHeaders();
    }
  ...
  _transform(message: MessageEvent, encoding: string, callback: (error?: Error | null, data?: any) => void) {
