import type { AdditionalHeaders, WritableHeaderStream } from "~/helpers/sse.helper.js";
...
import { isObject, Logger } from "@venok/core";

import { SseStream } from "~/helpers/sse.helper.js";
...
export class RouterResponseController {
  ...
  public async apply<TInput = any, TResponse = any>(result: TInput, response: TResponse, httpStatusCode?: number) {
  ...
  public setHeaders<TResponse = unknown>(response: TResponse, headers: CustomHeader[]) {
    headers.forEach(({ name, value }) => this.applicationRef.setHeader(response, name, value));
  }

  public setStatus<TResponse = unknown>(response: TResponse, statusCode: number) {
  ...
  >(result: TInput, response: TResponse, request: TRequest, options?: { additionalHeaders: AdditionalHeaders }) {
    ...
    const subscription = result
      .pipe(
        map((message): MessageEvent => {
          if (isObject(message)) return message as MessageEvent;

          return { data: message as object | string } as MessageEvent;
        }),
        debounce((message) => new Promise<void>((resolve) => stream.writeMessage(message, () => resolve()))),
        catchError((err) => {
          const data = err instanceof Error ? err.message : err;
          stream.writeMessage({ type: "error", data }, (writeError) => {
            if (writeError) this.logger.error(writeError);
          });
