...
import { ContextId } from "@venok/core";
import { ContextUtils } from "@venok/core/helpers/context.helper";
...
import { CUSTOM_ROUTE_ARGS_METADATA, FORBIDDEN_MESSAGE, ROUTE_ARGS_METADATA } from "@venok/core/constants";
...
export class HttpExecutionContext {
  ...
  public create(
    ...
    const {
      ...
      responseHeaders,
      hasCustomHeaders,
    } = this.getMetadata(instance, callback, methodName, moduleKey, requestMethod, contextType as ContextType);
    ...
    const handler =
      <TRequest, TResponse>(args: any[], req: TRequest, res: TResponse, next: Function) =>
      async () => {
  ...
  public getMetadata<TContext extends ContextType = ContextType>(
    ...
    const getParamsMetadata = (moduleKey: string, contextId = STATIC_CONTEXT, inquirerId?: string) =>
      this.exchangeKeysForValues(keys, metadata, moduleKey, contextId, inquirerId, contextFactory);
  ...
  public exchangeKeysForValues(
    ...
    return keys.map((key) => {
      ...
      if (key.includes(CUSTOM_ROUTE_ARGS_METADATA)) {
        // Maybe Error
        // @ts-ignore
        const { factory } = metadata[key as any];
        const customExtractValue = this.contextUtils.getCustomFactory(factory, data, contextFactory as any);
        return { index, extractValue: customExtractValue, type, data, pipes };
      }
      const numericType = Number(type);
      const extractValue = <TRequest, TResponse>(req: TRequest, res: TResponse, next: Function) =>
  ...
  public async getParamValue<T>(
    ...
    if (!isEmpty(pipes)) {
      return this.pipesConsumer.apply(value, { metatype, type, data } as any, pipes);
    }
    return value;
  }
  ...
  public createGuardsFn<TContext extends string = ContextType>(
    ...
    return (guards.length ? canActivateFn : null) as any;
  }

  public createPipesFn(pipes: PipeTransform[], paramsOptions: (ParamProperties & { metatype?: any })[]) {
    const pipesFn = async <TRequest, TResponse>(args: any[], req: TRequest, res: TResponse, next: Function) => {
      const resolveParamValue = async (param: ParamProperties & { metatype?: any }) => {
        const { index, extractValue, type, data, metatype, pipes: paramPipes } = param;
        const value = extractValue(req, res, next);

        args[index] = this.isPipeable(type)
          ? await this.getParamValue(value, { metatype, type, data } as any, pipes.concat(paramPipes))
          : value;
      };
