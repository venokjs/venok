...
import { addLeadingSlash, NoopHttpAdapter } from "../../helpers";
...
describe("middleware utils", () => {
  ...
  class Test {}
  function fnMiddleware(req: any, res: any, next: () => void) {}
  ...
  describe("mapToClass", () => {
    describe("when middleware is a class", () => {
      ...
      describe("when there are excluded routes", () => {
        it("should return a host class", () => {
          const type = mapToClass(Test, mapToExcludeRoute([{ path: "(.*)", method: RequestMethod.ALL }]), noopAdapter);
    ...
    describe("when middleware is a function", () => {
      ...
      it("should define a `use` method", () => {
        const metatype = mapToClass(fnMiddleware, [], noopAdapter) as Type;
        expect(new metatype().use).to.exist;
      });
      it("should encapsulate a function", () => {
        const spy = sinon.spy();
        const metatype = mapToClass(spy, [], noopAdapter) as Type;
        new metatype().use();
  ...
  describe("isRouteExcluded", () => {
    ...
    describe("when route is excluded", () => {
      ...
      it("should return true", () => {
        expect(isMiddlewareRouteExcluded({}, excludedRoutes, adapter)).to.be.true;
      });
    });
