...
import { HeaderStream, SseStream } from "@venok/http/helpers/sse.helper";
...
class Sink extends Writable implements HeaderStream {
  ...
  constructor(public readonly writeHead?: (statusCode: number, headers?: OutgoingHttpHeaders | string) => void) {
...
`,
  ...
  it("sets headers on destination when it looks like a HTTP Response", (callback) => {
    const sse = new SseStream();
    const sink: any = new Sink((status: number, headers: string | OutgoingHttpHeaders | any) => {
  ...
  it("allows an eventsource to connect", (callback) => {
    ...
    server.listen(() => {
      ...
      es.onerror = (e: any) => callback(new Error(`Error from EventSource: ${JSON.stringify(e)}`));
    });
  });
});
