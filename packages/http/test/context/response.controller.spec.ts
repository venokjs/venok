...
import { RouterResponseController } from "@venok/http/context/response.controller";
...
describe("RouterResponseController", () => {
    ...
    describe("when resultOrDeferred", () => {
      describe("is Promise", () => {
        it("should return Promise that resolves to the value resolved by the input Promise", async () => {
          const value = 100;
          expect(await routerResponseController.transformToResult(Promise.resolve(value))).to.be.eq(value);
        });
      });
      ...
      describe("is Observable", () => {
        it("should return toPromise", async () => {
          const lastValue = 100;
          expect(await routerResponseController.transformToResult(of(1, 2, 3, lastValue))).to.be.eq(lastValue);
        });
      });
      ...
      describe("is an object that has the method `subscribe`", () => {
        it("should return a Promise that resolves to the input value", async () => {
          ...
          expect(await routerResponseController.transformToResult(value)).to.equal(value);
        });
      });
      ...
      describe("is an ordinary value", () => {
        it("should return a Promise that resolves to the input value", async () => {
          const value = 100;
          expect(await routerResponseController.transformToResult(value)).to.be.eq(value);
        });
      });
    });
  });
  ...
  describe("getStatusByMethod", () => {
    describe("when RequestMethod is POST", () => {
      it("should return 201", () => {
        expect(routerResponseController.getStatusByMethod(RequestMethod.POST)).to.be.eql(201);
      });
    });
    describe("when RequestMethod is not POST", () => {
      it("should return 200", () => {
        expect(routerResponseController.getStatusByMethod(RequestMethod.GET)).to.be.eql(200);
      });
    });
  });
  ...
  describe("setHeaders", () => {
    ...
    it("should set all custom headers", () => {
      ...
      expect(setHeaderStub.calledWith(response, headers[0].name, headers[0].value)).to.be.true;
    });
  });
  ...
  describe("redirect should HttpServer.redirect", () => {
    it("should transformToResult", async () => {
      ...
      const result = {};
      await routerResponseController.redirect(result, null, null as any);
    ...
    it("should pass the response to redirect", async () => {
      ...
      const response = {};
      await routerResponseController.redirect(null, response, null as any);
    ...
    describe("status code", () => {
      ...
      it("should come from the redirectResponse if not on the transformed result", async () => {
        sinon.stub(routerResponseController, "transformToResult").returns(Promise.resolve({}));
        const redirectSpy = sinon.spy(adapter, "redirect");
        await routerResponseController.redirect(null, null, {
          statusCode: 123,
          url: "redirect url",
  ...
  describe("Server-Sent-Events", () => {
    it("should accept only observables", async () => {
      ...
      try {
        ...
        expect(e.message).to.eql("You must return an Observable stream to use Server-Sent Events (SSE).");
      }
    });
    ...
    it("should write string", async () => {
      ...
      const written = (stream: Writable) =>
        new Promise((resolve, reject) => stream.on("error", reject).on("finish", resolve));
