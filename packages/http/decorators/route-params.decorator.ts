...
import { RESPONSE_PASSTHROUGH_METADATA } from "@venok/http/constants";
...
export const Response: (options?: ResponseDecoratorOptions) => ParameterDecorator =
  (options?: ResponseDecoratorOptions) => (target, key, index) => {
    if (options?.passthrough) {
      Reflect.defineMetadata(RESPONSE_PASSTHROUGH_METADATA, options?.passthrough, target.constructor, key as any);
    }
