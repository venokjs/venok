...
export class RoutePathFactory {
  ...
  public create(metadata: RoutePathMetadata, requestMethod?: HttpMethod): string[] {
    let paths = [""];
        ...
        paths = flatten(
          paths.map((path) =>
            versionOrVersions.map((version) =>
              ...
              version === VERSION_NEUTRAL ? path : `${path}/${versionPrefix}${version}`
            )
          )
        );
      } else {
  ...
  public getVersionPrefix(versioningOptions: VersioningOptions): string {
    ...
    if (versioningOptions.type === HttpVersioningType.URI) {
      ...
      else if (versioningOptions.prefix !== undefined) return versioningOptions.prefix;
    }
