import {
  ...
  DEFAULT_METHOD_KEY,
} from "@venok/core/module/constants";
import { DynamicModule, InjectionToken, Provider } from "@venok/core/interfaces";
...
> {
  ...
  constructor(
    ...
    if (parentBuilder) {
      ...
      this.transformModuleDefinition = parentBuilder.transformModuleDefinition as any;
  ...
  > {
    ...
    class InternalModuleClass {
      static [self.staticMethodKey](options: ModuleOptions & ExtraModuleDefinitionOptions): DynamicModule {
        const providers: Array<Provider> = [
          {
            provide: self.options.optionsInjectionToken as InjectionToken,
      ...
      private static createAsyncProviders(options: ConfigurableModuleAsyncOptions<ModuleOptions>): Provider[] {
      ...
      private static createAsyncOptionsProvider(options: ConfigurableModuleAsyncOptions<ModuleOptions>): Provider {
        ...
        return {
          provide: self.options.optionsInjectionToken as InjectionToken,
          useFactory: async (optionsFactory: ConfigurableModuleOptionsFactory<ModuleOptions, FactoryClassMethodKey>) =>
  ...
  private createTypeProxy(typeName: "OPTIONS_TYPE" | "ASYNC_OPTIONS_TYPE" | "OptionsFactoryInterface") {
