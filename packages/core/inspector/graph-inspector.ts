...
export class GraphInspector {
  ...
  private readonly enhancersMetadataCache = new Array<EnhancerMetadataCacheEntry>();

  constructor(private readonly container: VenokContainer) {
    this.graph = container.serializedGraph;
  }

  public inspectModules(modules: Map<string, Module> = this.container.getModules()) {
  ...
  public insertAttachedEnhancer(wrapper: InstanceWrapper) {
    const existingNode = this.graph.getNodeById(wrapper.id) as Node;
    existingNode.metadata.global = true;

    this.graph.insertAttachedEnhancer(existingNode.id);
  }

  public insertEntrypointDefinition<T>(definition: Entrypoint<T>, parentId: string) {
    definition = {
      ...definition,
  ...
  private insertEnhancerEdge(entry: EnhancerMetadataCacheEntry) {
    ...
    const sourceInstanceWrapper = moduleRef.providers.get(entry.classRef) as InstanceWrapper;
    const existingSourceNode = this.graph.getNodeById(sourceInstanceWrapper.id) as ClassNode;
    ...
    if (entry.enhancerInstanceWrapper) {
      ...
      const name = entry.enhancerRef!.constructor?.name ?? (entry.enhancerRef as Function).name;
