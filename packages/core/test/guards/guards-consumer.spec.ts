...
import { of } from "rxjs";
import { GuardsConsumer } from "@venok/core/guards";
  ...
  describe("tryActivate", () => {
    describe("when guards array is empty", () => {
      it("should return true", async () => {
        const canActivate = await consumer.tryActivate([], [], { constructor: null }, null as any);
    ...
    describe("when guards array is not empty", () => {
      ...
      describe("when each guard returns true", () => {
        it("should return true", async () => {
          const canActivate = await consumer.tryActivate(guards, [], { constructor: null }, null as any);
          expect(canActivate).to.be.true;
        });
      });
    });
