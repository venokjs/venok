...
import { InterceptorsContextCreator } from "@venok/core/interceptors";
...
describe("InterceptorsContextCreator", () => {
  ...
  beforeEach(() => {
    ...
    interceptorsContextCreator = new InterceptorsContextCreator(container as any, applicationConfig);
  });
  describe("createConcreteContext", () => {
    ...
    describe("when `moduleContext` is defined", () => {
      ...
      it("should filter metatypes", () => {
        ...
        expect(interceptorsContextCreator.createConcreteContext(interceptorTypeRefs)).to.have.length(2);
      });
    });
  });
  ...
  describe("getInterceptorInstance", () => {
    describe("when param is an object", () => {
      it("should return instance", () => {
        ...
        expect(interceptorsContextCreator.getInterceptorInstance(instance as any)).to.be.eql(instance);
      });
    });
    describe("when param is a constructor", () => {
      it("should pick instance from container", () => {
        ...
        sinon.stub(interceptorsContextCreator, "getInstanceByMetatype").callsFake(() => wrapper);
        expect(interceptorsContextCreator.getInterceptorInstance(Interceptor)).to.be.eql(wrapper.instance);
      });
      it("should return null", () => {
        sinon.stub(interceptorsContextCreator, "getInstanceByMetatype").callsFake(() => null as any);
        expect(interceptorsContextCreator.getInterceptorInstance(Interceptor)).to.be.eql(null);
      });
    });
  });
  ...
  describe("getInstanceByMetatype", () => {
    describe('when "moduleContext" is nil', () => {
      it("should return undefined", () => {
        ...
        expect(interceptorsContextCreator.getInstanceByMetatype(null as any)).to.be.undefined;
      });
    });
    describe('when "moduleContext" is not nil', () => {
      ...
      describe("and when module exists", () => {
        it("should return undefined", () => {
          expect(interceptorsContextCreator.getInstanceByMetatype(class {})).to.be.undefined;
        });
      });
    });
  });
  ...
  describe("getGlobalMetadata", () => {
    describe("when contextId is static and inquirerId is nil", () => {
      it("should return global interceptors", () => {
        ...
        expect(interceptorsContextCreator.getGlobalMetadata()).to.be.equal(expectedResult);
      });
    });
    describe("otherwise", () => {
      it("should merge static global with request/transient scoped interceptors", () => {
        ...
        sinon.stub(applicationConfig, "getGlobalInterceptors").callsFake(() => globalInterceptors);
        sinon.stub(applicationConfig, "getGlobalRequestInterceptors").callsFake(() => scopedInterceptorWrappers);
        sinon.stub(instanceWrapper, "getInstanceByContextId").callsFake(() => ({ instance }) as any);

        expect(interceptorsContextCreator.getGlobalMetadata({ id: 3 })).to.contains(instance, ...globalInterceptors);
      });
    });
  });
});
