...
import { Module as ModuleDecorator } from "@venok/core/decorators/module.decorator";
...
import { RuntimeException, UnknownElementException, UnknownExportException } from "@venok/core/errors/exceptions";
...
describe("Module", () => {
  ...
  describe("addCustomUseExisting", () => {
    ...
    it("should store provider", () => {
      ...
      const factoryFn = (module as any)._providers.get(provider.provide).metatype;
  ...
  describe("replace", () => {
    describe("when provider", () => {
      it("should call `mergeWith`", () => {
        ...
        module.replace(null as unknown as InjectionToken, { isProvider: true });
    ...
    describe("when guard", () => {
      it("should call `mergeWith`", () => {
        ...
        module.replace(null as unknown as InjectionToken, {});
        expect(wrapper.mergeWith.called).to.be.true;
      });
    });
  });
  ...
  describe("validateExportedProvider", () => {
    ...
    describe("when unit exists in related modules collection", () => {
      it("should behave as identity", () => {
        class Random {}
        (module as any)._imports = new Set([new Module(Random, new VenokContainer())]);
    ...
    describe("when unit does not exist in both provider and related modules collections", () => {
      it("should throw UnknownExportException", () => {
        expect(() => module.validateExportedProvider(token)).to.throws(UnknownExportException);
      });
    });
  });
