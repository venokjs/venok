  ...
  describe("[static methods]", () => {
    describe("when the default logger is used", () => {
      ...
      it("should print one message to the console", () => {
        ...
        expect(processStdoutWriteSpy.firstCall.firstArg).to.include(`[${context}]`);
      ...
      it("should print multiple messages to the console", () => {
        ...
        expect(processStdoutWriteSpy.firstCall.firstArg).to.include(`[${context}]`);
        expect(processStdoutWriteSpy.firstCall.firstArg).to.include(messages[0]);

        expect(processStdoutWriteSpy.secondCall.firstArg).to.include(`[${context}]`);
        expect(processStdoutWriteSpy.secondCall.firstArg).to.include(messages[1]);

        expect(processStdoutWriteSpy.thirdCall.firstArg).to.include(`[${context}]`);
        expect(processStdoutWriteSpy.thirdCall.firstArg).to.include(messages[2]);
      });
      ...
      it("should print one error to the console with context", () => {
        ...
        expect(processStderrWriteSpy.firstCall.firstArg).to.include(`[${context}]`);
      ...
      it("should print one error to the console with stacktrace", () => {
        ...
        expect(processStderrWriteSpy.firstCall.firstArg).to.include(message);
        expect(processStderrWriteSpy.secondCall.firstArg).to.equal(stacktrace + "\n");
      });
      ...
      it("should print error object without context to the console", () => {
        ...
        expect(processStderrWriteSpy.calledOnce).to.be.true;
        expect(processStderrWriteSpy.firstCall.firstArg).to.include(`Error: Random text here`);
      });
      ...
      it("should print one error with stacktrace and context to the console", () => {
        ...
        expect(processStderrWriteSpy.secondCall.firstArg).to.equal(stacktrace + "\n");
        expect(processStderrWriteSpy.secondCall.firstArg).to.not.include(context);
      });
      ...
      it("should print multiple 2 errors and one stacktrace to the console", () => {
        ...
        expect(processStderrWriteSpy.firstCall.firstArg).to.include(`[${context}]`);
        expect(processStderrWriteSpy.firstCall.firstArg).to.include(messages[0]);

        expect(processStderrWriteSpy.secondCall.firstArg).to.include(`[${context}]`);
        expect(processStderrWriteSpy.secondCall.firstArg).to.include(messages[1]);

        expect(processStderrWriteSpy.thirdCall.firstArg).to.not.include(`[${context}]`);
    ...
    describe("when logging is disabled", () => {
      ...
      beforeEach(() => {
        ...
        previousLoggerRef = Logger["staticInstanceRef"];
  ...
  describe("ConsoleLogger", () => {
    ...
    describe("functions for message", () => {
      ...
      it("works", () => {
        ...
        expect(processStdoutWriteSpy.firstCall.firstArg).not.to.include("function");
        expect(processStdoutWriteSpy.firstCall.firstArg).not.to.include("Function");
      });
    });
  });
  ...
  describe("[instance methods]", () => {
    ...
    describe("when the default logger is used and global context is set and timestamp enabled", () => {
      ...
      it("should print multiple messages to the console and append global context", () => {
        ...
        expect(processStdoutWriteSpy.calledThrice).to.be.true;
        expect(processStdoutWriteSpy.firstCall.firstArg).to.include(`[${globalContext}]`);
        expect(processStdoutWriteSpy.firstCall.firstArg).to.include(messages[0]);

        expect(processStdoutWriteSpy.secondCall.firstArg).to.include(`[${globalContext}]`);
        expect(processStdoutWriteSpy.secondCall.firstArg).to.include(messages[1]);
      ...
      it("should log out an error to stderr but not include an undefined log", () => {
        ...
        expect(processStderrWriteSpy.calledOnce).to.be.true;
        expect(processStderrWriteSpy.firstCall.firstArg).to.include(`[${globalContext}]`);
        expect(processStderrWriteSpy.firstCall.firstArg).to.include(message);
      });
    });
    ...
    describe("when custom logger is being used", () => {
      ...
      describe("with global context", () => {
        ...
        it('should call custom logger "#log()" method with context as second argument', () => {
          ...
          expect(customLoggerLogSpy.called).to.be.true;
          expect(customLoggerLogSpy.calledWith(message, globalContext)).to.be.true;
        });
        it('should call custom logger "#error()" method with context as third argument', () => {
          ...
          expect(customLoggerErrorSpy.called).to.be.true;
          expect(customLoggerErrorSpy.calledWith(message, undefined, globalContext)).to.be.true;
        });
      });
      describe("without global context", () => {
        ...
        it('should call custom logger "#error()" method', () => {
          ...
          expect(customLoggerErrorSpy.called).to.be.true;
          expect(customLoggerErrorSpy.calledWith(message, undefined, context)).to.be.true;
        });
      });
    });
  });
