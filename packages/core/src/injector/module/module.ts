...
export class Module {
  ...
  private readonly _providers = new Map<InjectionToken, InstanceWrapper<InjectableToken>>();
  private readonly _injectables = new Map<InjectionToken, InstanceWrapper<InjectableToken>>();
  ...
  get entryProviders(): Array<InstanceWrapper<InjectableToken>> {
    return Array.from(this._entryProviderKeys)
      ...
      .filter(Boolean) as InstanceWrapper<InjectableToken>[];
  }
  ...
  get instance(): VenokModule {
    ...
    const module = this._providers.get(this._metatype) as InstanceWrapper;
  ...
  public addProvider(provider: Provider, enhancerSubtype?: EnhancerSubtype) {
    ...
    this._providers.set(
      ...
      new InstanceWrapper({
        token: provider,
        name: (provider as Type<InjectableToken>).name,
        metatype: provider as Type<InjectableToken>,
        instance: null,
        isResolved: false,
    ...
    return provider as Type<InjectableToken>;
  }
  ...
  public isCustomValue(provider: any): provider is ValueProvider {
    return isObject(provider) && Object.prototype.hasOwnProperty.call(provider, "useValue");
  }
  ...
  public getProviderById<T = any>(id: string): InstanceWrapper<T> | undefined {
    return Array.from(this._providers.values()).find((item) => item.id === id) as InstanceWrapper<T>;
  }
  ...
  public createModuleReferenceType(): Type<ModuleRef> {
    ...
    return class extends ModuleRef {
      ...
      public resolve<TInput = any, TResult = TInput>(
        ...
        return this.resolvePerContext<TInput, TResult>(typeOrToken, self, contextId, options);
      }

      public async create<T = any>(type: Type<T>, contextId?: ContextId): Promise<T> {
  ...
  private isEntryProvider(metatype: InjectionToken): boolean {
    return typeof metatype === "function" ? !!Reflect.getMetadata(ENTRY_PROVIDER_WATERMARK, metatype) : false;
  }
