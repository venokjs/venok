...
describe("ConfigurableModuleBuilder", () => {
  describe("setExtras", () => {
    it("should apply module definition transformer function and return typed builder", () => {
      const { ConfigurableModuleClass } = new ConfigurableModuleBuilder()
        .setExtras({ isGlobal: false }, (definition, extras: { isGlobal: boolean }) => ({
          ...definition,
  ...
  describe("setFactoryMethodName", () => {
    it("should set configuration factory class method name and return typed builder", () => {
      const { ConfigurableModuleClass } = new ConfigurableModuleBuilder().setFactoryMethodName("createOptions").build();
    ...
    it('should return a fully typed "ConfigurableModuleClass"', () => {
      ...
      const { ConfigurableModuleClass, OPTIONS_TYPE, ASYNC_OPTIONS_TYPE, MODULE_OPTIONS_TOKEN } =
        new ConfigurableModuleBuilder({
          ...
          .setExtras<ExtraConfig>({ isGlobal: false, extraProviders: [] }, (definition, extras) => ({
            ...definition,
            global: extras.isGlobal,
      ...
      expect(definition.providers).to.include.members(provideInjectionTokensFrom.slice(0, 2));
      expect(definition.providers).not.to.include(provideInjectionTokensFrom[2]);
      expect(MODULE_OPTIONS_TOKEN).to.equal("RANDOM_TEST_MODULE_OPTIONS");
      expect((definition.providers![0] as any).provide).to.equal("RANDOM_TEST_MODULE_OPTIONS");

      try {
        expect(ASYNC_OPTIONS_TYPE.imports).to.equal(undefined);
      } catch (err) {
        expect(err).to.be.instanceOf(Error);
        expect((err as any).message).to.equal('"ASYNC_OPTIONS_TYPE" is not supposed to be used as a value.');
      }
      try {
        expect(OPTIONS_TYPE.isGlobal).to.equal(undefined);
      } catch (err) {
        expect(err).to.be.instanceOf(Error);
        expect((err as any).message).to.equal('"OPTIONS_TYPE" is not supposed to be used as a value.');
      }
    });
  });
});
